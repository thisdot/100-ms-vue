<template>
  <div>
    <div v-if="networkScore > 0" class="network-container">
      <svg
        :width="12"
        :height="12"
        viewBox="0 0 14 12"
    ...
    <div v-else class="poor network-container">
      <svg
        width="12px"
        height="12px"
        viewBox="0 0 24 24"
...
onMounted(() => {
  return hmsStore.subscribe((score) => {
    networkScore.value = score?.downlinkQuality || -1;
  }, selectConnectionQualityByPeerID(props.peerId));
});
</script>
